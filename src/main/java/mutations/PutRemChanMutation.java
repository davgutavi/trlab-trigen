...
public class PutRemChanMutation implements Mutation {
	...
	public void mutate(AlgorithmIndividual individual) {
		...
		applyAlteration (individual,randomSupport.getFromInterval(1,9));
	...
	private void applyAlteration(AlgorithmIndividual individual, int type) {
		...
		else if (type==3) quitar_tiempo.alter(individual);
		else if (type==4) poner_gen.alter(individual);
		else if (type==5) poner_condicion.alter(individual);
		else if (type==6) poner_tiempo.alter(individual);	
		else if (type==7) cambiar_gen.alter(individual);
		else if (type==8) cambiar_condicion.alter(individual);
		else if (type==9) cambiar_tiempo.alter(individual);	
